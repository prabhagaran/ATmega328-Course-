 ...
 * 3. External interrupt INT0 is configured to trigger on a rising edge (high level) of the button signal.
 * 4. When the button connected to PD2 is pressed (high level), an interrupt is triggered.
 * 5. The ISR (Interrupt Service Routine) checks the state of PD2:
 *    - If PD2 is high (button pressed), the LED is turned on.
 *    - If PD2 is low (button released), the LED is turned off.
